...
export const DEFAULT_THEMES: Record<string, ThemeJson> = {
  ...
  promptcode: generatePromptCodeTheme(),
  orng,
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    const values = createMemo(() => {
      return resolveTheme(store.themes[store.active] ?? store.themes.promptcode, store.mode)
    })
    ...
    return {
      ...
      setMode(mode: "dark" | "light") {
        if (mode !== "dark") {
          console.warn("Only dark mode is supported - PromptCode light-blue theme is enforced")
          return
        }
        setStore("mode", "dark")
...
function generatePromptCodeTheme(): ThemeJson {
  const primary = RGBA.fromHex("#60a5fa")
  const accent = RGBA.fromHex("#3b82f6")
  const secondary = RGBA.fromHex("#93c5fd")
  const background = RGBA.fromHex("#0f172a")
  const text = RGBA.fromHex("#e0f2fe")
  const success = RGBA.fromHex("#34d399")
  const error = RGBA.fromHex("#f87171")
  const warning = RGBA.fromHex("#fbbf24")
  const info = RGBA.fromHex("#60a5fa")
  const textMuted = RGBA.fromHex("#94a3b8")
  const transparent = RGBA.fromInts(0, 0, 0, 0)
  const backgroundPanel = RGBA.fromHex("#1e293b")
  const backgroundElement = RGBA.fromHex("#334155")
  const backgroundMenu = RGBA.fromHex("#334155")
  const border = RGBA.fromHex("#475569")
  const borderActive = RGBA.fromHex("#60a5fa")
  const borderSubtle = RGBA.fromHex("#334155")

  const diffAlpha = 0.22
  const diffAddedBg = tint(background, success, diffAlpha)
  const diffRemovedBg = tint(background, error, diffAlpha)
  const diffAddedLineNumberBg = tint(backgroundElement, success, diffAlpha)
  const diffRemovedLineNumberBg = tint(backgroundElement, error, diffAlpha)
  ...
  return {
    theme: {
      ...
      diffHighlightAdded: RGBA.fromHex("#6ee7b7"),
      diffHighlightRemoved: RGBA.fromHex("#fca5a5"),
      diffAddedBg,
      diffRemovedBg,
      diffContextBg: RGBA.fromHex("#1e293b"),
      diffLineNumber: RGBA.fromHex("#64748b"),
      diffAddedLineNumberBg,
      diffRemovedLineNumberBg,
      markdownText: text,
      markdownHeading: primary,
      markdownLink: secondary,
      markdownLinkText: RGBA.fromHex("#bae6fd"),
      ...
      markdownListEnumeration: RGBA.fromHex("#bae6fd"),
      markdownImage: secondary,
      markdownImageText: RGBA.fromHex("#bae6fd"),
      ...
      syntaxType: primary,
      syntaxOperator: RGBA.fromHex("#bae6fd"),
