...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run PromptCode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for PromptCode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found PromptCode session: ${sessionUrl}`)
        ...
        UI.println()
        UI.println("Starting PromptCode...")
        UI.println()

        // Launch opencode TUI with session ID if available
        const { spawn } = await import("child_process")
        const promptcodeArgs = sessionId ? ["-s", sessionId] : []
        const promptcodeProcess = spawn("promptcode", promptcodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          promptcodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`promptcode exited with code ${code}`))
          })
          promptcodeProcess.on("error", reject)
        })
      },
    })
  },
